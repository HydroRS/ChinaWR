...
%% 读取自然流域信息（也可以替换成行政区数据）
[County_infor, County_name] = xlsread('C:\自然流域\info.xls');
province_id = unique(County_infor(:, 1)); % 没有id就按照FID
shp_pro_file =  'C:\自然流域\shp_exarea.shp';
...
for year_idx = 1:length(years)
    ...
    for month_idx = 1:length(months)
        ...
        fileName = sprintf('data_%d_%02d.tif', year, month);
        ...
        if exist(filePath, 'file')
            ...
            for kk = 1:length(province_id)
                ...
                if all(data_County(:) == 0)
                    ...
                    data_County(data_County == -32768) = 0;
